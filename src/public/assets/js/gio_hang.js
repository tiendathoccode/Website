const productDetail = {
  ...
  images: {
    ...
    thumb2:
      "https://images.unsplash.com/photo-1611085583191-a3b181a88401?w=900&auto=format&fit=crop&q=80",
  },
};
...
function renderCartItems() {
  ...
  if (cartItems.length === 0) {
    cartItemList.innerHTML =
      '<p style="color:var(--color-muted);font-size:12px;text-align:center;padding-top:40px;">Giỏ hàng của bạn đang trống.</p>';
    updateCartBadge();
    updateCartSubtotal();
    return;
  }
...
function addToCart() {
  ...
  const existingItem = cartItems.find(
    (i) => i.id === productDetail.id && i.metal === selectedMetal,
  );
...
thumbnailStrip.addEventListener("click", (e) => {
  ...
  if (thumbBtn)
    switchMainImage(thumbBtn.dataset.src, thumbBtn.dataset.alt, thumbBtn);
});
